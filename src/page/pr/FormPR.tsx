...
export default withRouter(({ history, match }: any) => {
  ...
  const save = async () => {
    ...
    let number: any = await getLastNumbering(
      "LPB",
      (selected[0] as any).WhsCode
    );
    setSaving(true);
    try {
      await APIPost("PurchaseReceipt", {
        ...data,
        U_IDU_GRPO_TRANSCODE: "LPB",
      ...
      if (e.Message.search("409") > -1) {
        updateLastNumbering(number.id, number.last_count + 1);
        alert("No PR sudah digunakan, simpan kembali untuk me-refresh No PR.");
      } else {
        alert(e.Message);
      }
        ...
        <UIJsonField
          ...
          field={[
            ...
            },
            {
              key: "optional",
              label: "Optional",
              value: [{ key: "Comments", label: "Remark", size: 12 }]
            }
          ]}
          ...
          tabs={[
            {
              ...
              action: (
                ...
                >
                  <IconCheck color="#fff" height={20} width={20} />
                  {isSize(["md", "lg"]) && (
